...
**先決條件**:專案**零憑證即可跑**。未設 `CWA_API_KEY` 時後端自動進 mock 模式,回可重現的假資料;前端亦有 mock fallback。設有 `CWA_API_KEY` 時,後端會依目標日期切到 CWA live dataset family: 近端走 `F-D0047-093` 邏輯族群(實際解析到各縣市 3 小時資料集),較遠日期走 `F-D0047-091` 邏輯族群(實際解析到各縣市 12 小時資料集)。
